#!/bin/bash
# Create ephemeral DVM templates in a zram-based pool (Qubes OS)
...
ZRAM_SIZE="20G"
VG_NAME="ram_vg"
WHONIX_TEMPLATE="whonix-workstation-18"
WHONIX_DVM="whonix-dvm"

# DVM templates that should NOT be cloned and should NOT be hidden
EXCLUDE_DVMS=(
    "fedora-d"
    "default-mgmt-dvm"
)

# Extra non-DVM qubes to clone into the ephemeral pool (optional)
EXTRA_VMS=(
    #"kicks"
    #"sys-whonix"
)
...
#######################################
# Amnesiac mode detection & cleanup
#######################################
...
if echo "$ROOT_DEV" | grep -qE "(overlay|/dev/zram0)"; then
    ...
    echo "[*] Cleaning up old VMs from pool ${POOL_NAME}..."
...
#######################################
# Restore visibility of original DVM templates
#######################################
echo "[*] Restoring visibility of original DVM templates..."
qvm-ls --raw-list 2>/dev/null | while read -r vm; do
    ...
    # Skip excluded DVMs
    skip=false
    for exclude in "${EXCLUDE_DVMS[@]}"; do
        if [ "$vm" = "$exclude" ]; then
            skip=true
            break
        fi
...
qvm-pool add "${POOL_NAME}" lvm_thin \
    --option volume_group="${VG_NAME}" \
    --option thin_pool=thin_pool 2>/dev/null || \
qvm-pool add "${POOL_NAME}" lvm_thin \
    -o volume_group="${VG_NAME}",thin_pool=thin_pool

echo " Pool created:"
qvm-pool info "${POOL_NAME}"

#######################################
# Clone DVM templates into ephemeral pool
#######################################
...
qvm-ls --raw-list 2>/dev/null | while read -r vm; do
    ...
    if qvm-clone -P "${POOL_NAME}" "$vm" "$clone_name"; then
        ...
        # Privacy features for the ephemeral clone
        qvm-features "$clone_name" net.fake-ip 192.168.1.100 2>/dev/null || true
        qvm-features "$clone_name" net.fake-gateway 192.168.1.1 2>/dev/null || true
        qvm-features "$clone_name" net.fake-netmask 255.255.255.0 2>/dev/null || true
        qvm-features "$clone_name" anon-timezone 1 2>/dev/null || true
        qvm-features "$clone_name" gui-events-max-delay 80 2>/dev/null || true

        # Hide the original DVM template
        echo " -> Hiding original: $vm"
...
#######################################
# Creating Whonix DVM template
#######################################
echo "[*] Creating Whonix DVM template..."

qvm-create -P "$POOL_NAME" -t "$WHONIX_TEMPLATE" -l purple --class AppVM "$WHONIX_DVM" || {
    echo "[!] Failed to create Whonix DVM"
    exit 1
}

qvm-prefs "$WHONIX_DVM" template_for_dispvms True
qvm-prefs "$WHONIX_DVM" netvm sys-whonix 2>/dev/null || true
qvm-prefs "$WHONIX_DVM" autostart False
qvm-prefs "$WHONIX_DVM" label purple
qvm-features "$WHONIX_DVM" appmenus-dispvm 1
qvm-features "$WHONIX_DVM" net.fake-ip 192.168.1.100 2>/dev/null || true
qvm-features "$WHONIX_DVM" net.fake-gateway 192.168.1.1 2>/dev/null || true
qvm-features "$WHONIX_DVM" net.fake-netmask 255.255.255.0 2>/dev/null || true
