...
Type=oneshot
RemainAfterExit=yes
ExecStartPre=-/usr/local/bin/ephemeral-pool-remove
ExecStart=/usr/local/bin/ephemeral-pool-create
ExecStop=/usr/local/bin/ephemeral-pool-remove

[Install]
WantedBy=multi-user.target
EOF

sudo tee /usr/local/bin/ephemeral-pool-create << 'EOF'
#!/bin/bash
set -uo pipefail
...
sudo tee /usr/local/bin/ephemeral-pool-remove << 'EOF'
#!/bin/bash
set -uo pipefail
...
sudo chmod +x /usr/local/bin/ephemeral-pool-create
sudo chmod +x /usr/local/bin/ephemeral-pool-remove
