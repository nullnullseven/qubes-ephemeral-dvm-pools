...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/tmpfs-pool-create
RemainAfterExit=yes
...
sudo tee /usr/local/bin/tmpfs-pool-create << 'EOF'
#!/bin/bash
set -uo pipefail
...
sudo tee /usr/local/bin/tmpfs-pool-remove << 'EOF'
#!/bin/bash
...
sudo chmod +x /usr/local/bin/tmpfs-pool-create
sudo chmod +x /usr/local/bin/tmpfs-pool-remove
